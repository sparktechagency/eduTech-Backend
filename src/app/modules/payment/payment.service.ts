...
const createPaymentCheckoutToStripe = async (user: JwtPayload, payload: any): Promise<string | null> => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        success_url: "https://www.admin.PROVIDERmeus.com/public/payment-success",
        cancel_url: "https://www.admin.PROVIDERmeus.com/public/payment-failed"
    });
...
const createAccountToStripe = async (user: JwtPayload) => {
    ...
    const accountLink = await stripe.accountLinks.create({
        account: account.id,
        refresh_url: "https://www.admin.PROVIDERmeus.com/public/onboard-success",
        return_url: "https://www.admin.PROVIDERmeus.com/public/onboard-failed",
...
const transferAndPayoutToPROVIDER = async (id: string) => {
    ...
    const isExistPROVIDER = await User.isAccountCreated(isExistReservation.PROVIDER);
    if (!isExistPROVIDER) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Sorry, you are didn't provide bank information. Please create a bank account");
    }

    //check completed payment and PROVIDER transfer
    ...
    const { accountId, externalAccountId } = isExistPROVIDER?.accountInformation;
    ...
    if (payouts.status === "paid") {
        ...
        const data = {
            ...
            receiver: isExistReservation.PROVIDER,
            referenceId: id,
            screen: "RESERVATION"
        }
