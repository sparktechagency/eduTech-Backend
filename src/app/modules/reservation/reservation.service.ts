...
const PROVIDERReservationFromDB = async (user: JwtPayload, query: Record<string, any>): Promise<any> => {
    ...
    const allStatus = await Promise.all(["Upcoming", "Accepted", "Canceled", "Completed"].map(
        async (status: string) => {
            return {
                status,
                count: await Reservation.countDocuments({ PROVIDER: user.id, status })
            }
        })
    );
...
const customerReservationFromDB = async (user: JwtPayload, query: Record<string, any>): Promise<{}> => {
    ...
    const reservations:any = await Reservation.find(condition)
        ...
        .select("PROVIDER service createdAt status travelFee appCharge tips price paymentStatus cancelByCustomer")
        .skip(skip)
        .limit(size)
        .lean();
...
const reservationSummerForPROVIDERFromDB = async (user: JwtPayload): Promise<{}> => {
    ...
    const todayEarnings = await Reservation.aggregate([
        {
            $match: { PROVIDER: user.id, createdAt: { $gte: new Date(today.getFullYear(), today.getMonth(), today.getDate()) } }
        },
        {
            $group: {
                _id: null,
    ...
    // total reservations
    const totalReservations = await Reservation.countDocuments({ PROVIDER: user.id } as any);
...
export const ReservationService = {
    ...
    PROVIDERReservationFromDB,
    customerReservationFromDB,
    reservationSummerForPROVIDERFromDB,
